...
| `draiverctl start <ticket[@attempt]>` | Hand one attempt to a running `ctl up` to bring up in the background (imperative + **transient**: a desired-marker stamped with the daemon's boot nonce, swept if it restarts). The daemon climbs the self-heal cascade — resume a live session id, else spawn fresh on the surviving worktree, else rebuild from `brief`. Requires `ctl up` (errors otherwise, never orphans); watch with `logs -f`. Contrast `enable` (declarative + persistent). `--new-attempt` forks a new attempt (new id, `from` provenance) and starts the fork, **leaving the parent running** — a parallel branch. |
| `draiverctl stop <ticket[@attempt]>` | Reap the session's process; keep id + worktree + log for a later `--resume`, and remove the desired-marker so the attempt leaves the imperative fleet. A **still-enabled** attempt is durably desired, so the daemon brings it back (a bare stop on it is transient); to durably stop, `disable`. Idempotent. |
| `draiverctl restart <ticket[@attempt]>` | Reap, flush volatile state to a chosen depth, then hand the attempt to `ctl up` to bring back up (same transient handoff as `start`; requires `ctl up`, returns immediately, watch with `logs -f`). Depth flags are cumulative (deepest wins): none = resume the same session (a clean continue, no re-brief); `--new-session` flushes the conversation (L0) → fresh session on the same worktree; `--new-worktree` also rebuilds the checkout from HEAD (L1, discards uncommitted work); `--new-attempt` **forks a new attempt** (L2, `from` provenance) and **parks the parent** (it leaves the fleet). Re-brief happens iff the session layer was flushed. |
...
| `draiverctl merge <ticket[@attempt]> [--dry-run] [--sync]` | Land the attempt's branch into its recorded `base:`, **fast-forward only** — the terminal of the lifecycle `ctl` owns. Gated to a **stopped Review attempt with a clean checkout** (drvctl-014 ethos); keeps the branch until the land is durable; on success records `done`, so Done means *the code is in the base*. A diverged branch refuses and points to `sync`; `--sync` runs sync-then-ff in one shot. `--dry-run` reports mergeability (`--is-ancestor` + `merge-tree`) without mutating. Never rebases, never forces. |
| `draiverctl merge <ticket[@attempt]> --remote[=NAME]` | **External twin** for a ticket landed by a PR merged on the forge (drvctl-029). Instead of landing the local branch it `git fetch`es `base:` from the remote (**read-only, never pushes** — the only place draiver reaches a remote), verifies the branch is **contained** in `refs/remotes/<remote>/<base>` (proof the PR merged; if not, refuses and records nothing, taking the `--escalate`/`--no-escalate` disposition), records `done`, then **best-effort** fast-forwards the local base. Gated to a stopped Review attempt but tolerates a dirty/diverged base — a failed local pull is a **warning**, never un-closing the ticket. Bare `--remote` uses the sole remote (or the config's `primary_remote` when several); `--remote=NAME` overrides. Never deletes the branch, never touches the forge API. |
| `draiverctl sync <ticket[@attempt]>` | Back-merge `base:` into the attempt's branch **additively** (one merge commit, SHAs preserved — no rebase), so a resume continues on the updated tip and the branch becomes ff-landable. Independently useful mid-attempt to pull the base's changes in. Materializes the checkout if it was reclaimed; requires it clean; aborts cleanly on conflict. |
| `draiverctl enable / disable <ticket[@attempt]>` | In-fleet / parked. Stored as an `enable`/`disable` log event (in the hash chain, replayed by `brief`). **Default is disabled**: `up` reconciles only attempts that are both `Running` *and* enabled, so a repo full of `Running` tickets never auto-spawns until each is opted in. `enable --now` also brings the attempt up immediately via a running `ctl up` (requires one) — the **persistent** counterpart of `start`: the enable bit survives a daemon restart where `start`'s transient marker is swept. (`mask` = never-auto-pick, deferred with the tournament.) |
